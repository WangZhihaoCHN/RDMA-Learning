...
int main(int argc, char *argv[])
{
	...
	for(memTmp=0;memTmp<4;++memTmp){
		ret = glex_register_mem(ep, sendbuff[memTmp], sizeof(char)*buffsize, GLEX_MEM_READ|GLEX_MEM_WRITE, &mh[memTmp]);
		ret = glex_register_mem(ep, recvbuff[memTmp], sizeof(char)*buffsize, GLEX_MEM_READ|GLEX_MEM_WRITE, &remote_mem_addr[memTmp]);
	}
...
//		.coll_set_num = ,
	...
	if(my_id == 0){
		...
		printf("GLEX_HALO_EXCHANGE(RDMA_PUT): Trans %d Bytes,",buffsize);
