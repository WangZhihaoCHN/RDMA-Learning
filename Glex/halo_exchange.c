...
int main(int argc,char** argv)
{
    ...
    if((taskid-col)>=0){
    	...
    	ierr = MPI_Isend(sendbuff,buffsize,MPI_DOUBLE,taskid+(col-1)*row,0,MPI_COMM_WORLD,&send_request3);
    	ierr = MPI_Irecv(recvbuff3,buffsize,MPI_DOUBLE,taskid+(col-1)*row,MPI_ANY_TAG,MPI_COMM_WORLD,&recv_request3);
    }
    // 向下进程发送与接收
    if((taskid+col)<ntasks){
    	...
    	ierr = MPI_Isend(sendbuff,buffsize,MPI_DOUBLE,taskid-(col-1)*row,0,MPI_COMM_WORLD,&send_request4);
    	ierr = MPI_Irecv(recvbuff4,buffsize,MPI_DOUBLE,taskid-(col-1)*row,MPI_ANY_TAG,MPI_COMM_WORLD,&recv_request4);
    }
