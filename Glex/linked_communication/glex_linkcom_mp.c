...
int main(int argc, char *argv[])
{
	...
	if(my_id == num_procs-1) {
		...
		struct glex_imm_mp_req mpReq = {
			...
			.coll_counter = 0,
			.len = sizeof(char) * 10,
			.flag = (GLEX_FLAG_COLL|GLEX_FLAG_COLL_SEQ_START|GLEX_FLAG_COLL_SEQ_TAIL|GLEX_FLAG_COLL_COUNTER|GLEX_FLAG_COLL_CP_DATA|GLEX_FLAG_COLL_CP_SWAP_DATA),
			.next = NULL
		};
		ret = glex_send_imm_mp(ep, &mpReq, NULL);
		TEST_RetSuccess(ret, "非阻塞发送MP报文失败!");
	} else {
		//strcpy(mem_addr, "World!\n");
		struct glex_imm_mp_req mpReq = {
			.rmt_ep_addr = remote_ep_addr,
			.data = mem_addr,
			.coll_counter = 1,
			.len = sizeof(char) * 10,
			.flag = (GLEX_FLAG_COLL|GLEX_FLAG_COLL_SEQ_START|GLEX_FLAG_COLL_SEQ_TAIL|GLEX_FLAG_COLL_COUNTER|GLEX_FLAG_COLL_CP_DATA|GLEX_FLAG_COLL_CP_SWAP_DATA|GLEX_FLAG_COLL_SWAP),
			.next = NULL
		};
		ret = glex_send_imm_mp(ep, &mpReq, NULL);
		TEST_RetSuccess(ret, "非阻塞发送MP报文失败!");
