...
int main(int argc, char *argv[])
{
	...
	if(my_id == 0){		//发送进程
		MPI_Isend(msg, strlen(msg), MPI_CHAR, 1, 1, MPI_COMM_WORLD, &handle);
		MPI_Wait(&handle,&status);
	}else if(my_id == 1){		
		MPI_Recv(remoteMsg, strlen(msg), MPI_CHAR, 0, 1, MPI_COMM_WORLD, &status);
		MPI_Isend(msg, strlen(msg), MPI_CHAR, 2, 0, MPI_COMM_WORLD, &handle);
		MPI_Wait(&handle,&status);
		MPI_Isend(msg, strlen(msg), MPI_CHAR, 3, 0, MPI_COMM_WORLD, &handle);
		MPI_Wait(&handle,&status);
	}else if (my_id == 2){
	...
	if(my_id == 0) {
		...
		struct glex_imm_mp_req mpReq = {
			...
			.coll_counter = 3,
			.len = sizeof(char) * 10,
			.flag = (GLEX_FLAG_COLL|GLEX_FLAG_COLL_SEQ_START|GLEX_FLAG_COLL_SEQ_TAIL|GLEX_FLAG_COLL_COUNTER|GLEX_FLAG_COLL_CP_DATA),
			.next = NULL
		};
		...
		glex_ep_addr_t source_ep_addr;
		uint32_t mpLen;
		printf("进程1接收前，buffer内容是：%s\n", mem_addr);
		ret = glex_receive_mp(ep, -1, &source_ep_addr, mem_addr, &mpLen);
		TEST_RetSuccess(ret, "阻塞接收MP报文失败！");
		printf("进程1接收后，buffer内容是，%s\n", mem_addr);
	} else if (my_id == 2) {
