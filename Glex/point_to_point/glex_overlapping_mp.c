...
int main(int argc, char *argv[])
{
	...
	/* 注册内存，锁内存并建立映射关系 */
	char *mem_addr = (char *)malloc(sizeof(char)*buffersize);
	glex_mem_handle_t mh;
	ret = glex_register_mem(ep, mem_addr, sizeof(char)*buffersize, GLEX_MEM_READ|GLEX_MEM_WRITE, &mh);
	...
	if(my_id == 0){
		endTime = MPI_Wtime();
		totalTime = (endTime - startTime) * 1e6 / 2000;
		printf("MPI_P2P: Trans %d Bytes,",buffersize);
